...
function runLassos(VIX, raw, expTrans, timeTrans, TA, trainingSize)
    ...
    println("Predicting industry ", industry)
    summary = CSV.read(path*industry*"/"*"Summary "*string(trainingSize)*".csv", nullable=false)
    testMat = zeros(845,372)
    i = 22 # i = 22 = VIXMacroTimeTA
	fileName = summary[i,1] #Fetch dataset from summary
    ...
    tempMat = CSV.read(path*industry*"/"*string(trainingSize)*"-1/240_"*fileName*"_bMatrix240_"*string(bestGamma)*".csv",
	...
	if length(x) < 10
		x = find(x-> x > 0.20, mean(allMat,1))
	end
	if length(x) < 10
		x = find(x-> x > 0.15, mean(allMat,1))
	end
	if length(x) < 10
		x = find(x-> x > 0.10, mean(allMat,1))
	end
	if length(x) < 10
		x = find(x-> x > 0.075, mean(allMat,1))
	end
	...
	fileName = fileName*"_over25PercentPredictionTermsDyn"
